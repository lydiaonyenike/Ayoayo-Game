...
def display(board: list):
    if not board:
        return
    print()
    print(' ', *[i for i in range(11, 5, -1)], ' ', sep="\t")
    print('-' * 57)
    print('|', *board[11:5:-1], '|', sep="\t")
    print('|', '-' * 53, '|')
    print('|', *board[0:6], '|', sep="\t")
    print('-' * 57)
...
def is_end(board: list, turn: int, players: list, previous_states) -> bool:
    
    state_ids = [''.join([str(j) for j in i]) for i in previous_states]
    
    if max(players) >= 25:  # 25 win rule
        return True
    if abs(players[0] - players[1]) > sum(board):
        return True
    if players[0] == players[1] == 24:
        return True
    if sum(board) <= 3:
        return True
    current_id = ''.join([str(i) for i in board])
    if(current_id in state_ids):
        return True
